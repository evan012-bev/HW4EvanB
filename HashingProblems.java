...
import java.util.Set;
 ...
 class HashingProblems {
    ...
    for (Map.Entry<Integer, String> entry : map.entrySet()) {
        if (entry.getKey() % 2 != 0) { // Check if the key is odd
            result.add(entry.getValue()); // Add corresponding value to result list
        }
    }
    ...
    return result;
      ...
      */
      ...
      public int twoSums(int[] numbers, int k) {
        ...
        // Populate the HashSet with all numbers in the array
        ...
        // Iterate through the array to check for valid differences
        for (int number : numbers) {
            if (set.contains(number - k)) { // Check if the required difference exists
                res++;
            }
        }
