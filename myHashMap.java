 ...
 class myHashMap<K, V> {
      ...
      */
      ...
      public V remove(K key) {
        ...
        int index = getBucketIndex(key); // Get bucket index for the given key
        HashNode<K, V> head = bucket.get(index);
        HashNode<K, V> prev = null;
    
        // Traverse the linked list at the bucket index to find the key
        while (head != null) {
            if (head.key.equals(key)) {
                V removedValue = head.value; // Store the value to return
                
                // Remove the node by adjusting references
                if (prev == null) {
                    bucket.set(index, head.next); // Removing head node
                } else {
                    prev.next = head.next; // Removing non-head node
                }
      ...
      */
      ...
      public V replace(K key, V val) {
        ...
        int index = getBucketIndex(key); // Determine the bucket index for the key
        HashNode<K, V> head = bucket.get(index); // Retrieve the head node at this bucket index
    
        // Iterate through the linked list at this bucket index
        while (head != null) {
            if (head.key.equals(key)) {
                V oldValue = head.value; // Store the current value before replacing it
      ...
      */
      ...
      public boolean replace(K key, V oldVal, V newVal) {
        ...
        int index = getBucketIndex(key); // Determine the bucket index for the key
        HashNode<K, V> head = bucket.get(index); // Retrieve the head node at this index
    
        // Traverse the linked list at this bucket index
        while (head != null) {
            if (head.key.equals(key) && head.value.equals(oldVal)) {
                replace(key, newVal); // Reuse the existing replace method
                return true; // Replacement successful
            }
            head = head.next; // Move to the next node
        }
    
        return false; // Key-value pair not found, no replacement occurred
    }
